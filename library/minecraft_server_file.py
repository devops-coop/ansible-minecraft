...
class BadValuesTypeException(Exception):
    def __init__(self, expected_type, actual_type):
        super(BadValuesTypeException, self).__init__(
            'expected "values" to be {} but got {}'.format(expected_type, actual_type)
        )
...
class FileStats(object):
    ...
    def __init__(self, module):
        self.module = module
        missing_args = [x for x in self.REQUIRED_ARGS if not self.module.params[x]]
        if missing_args:
    ...
    def __init__(self, module):
        ...
        with open(self.stats.path) as filein:
            for line in filein:
                ...
                name, eq, val = [x.strip() for x in line.strip().partition("=")]
...
def main(argv=None):
    module = AnsibleModule(
        argument_spec=dict(
            server_file=dict(required=True, choices=SERVER_FILE_CHOICES, type="str"),
    ...
    if module.check_mode:
        module.exit_json(
            content=server_file.content, path=server_file.stats.path, changed=changed
        )
        return
